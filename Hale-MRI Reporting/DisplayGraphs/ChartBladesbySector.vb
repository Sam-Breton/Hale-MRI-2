...
#Region "Types and Constants"
    ...
    Private Const kChartTitle As String = "Blades By Sector"
    Private Const kSeriesName As String = "Segment"
    Private Const kPointName As String = "Blade "
...
#Region "Private Interface"
    Private Sub BasisSet(basisValue As String, pitchValue As Double?, radiusValue As Double?)
        If basisValue IsNot Nothing AndAlso pitchValue IsNot Nothing AndAlso radiusValue IsNot Nothing Then
            ...
            Chart1.Titles("SecondaryTitle").Text = $"Radius {radiusValue.Value} - Compare to {basisValue}"
        End If
    End Sub
    ...
    Protected Overrides Sub DataGet()
        ' Get any data required by this DisplayControl.
        Me.Radius = If(Me.Radius, 50.0#) ' Need a way to get this from the user - maybe a dropdown on the form?
        MyBase.DataGet()
    End Sub
    ...
    Protected Overrides Sub DataShow()
        If mDisplayInitialized Then
            ...
            If Me.BladeCount IsNot Nothing AndAlso
                ...
                Chart1.Titles("SecondaryTitle").Text = $"Radius {Math.Round(Radius.Value, 2)} - Compare to {Basis}"
                For x As Integer = 1 To LocalPitchSectors
                    ...
                    For y As Integer = 1 To BladeCount
#Disable Warning BC42324 ' Using the iteration variable in a lambda expression may have unexpected results
                        Dim bladeData As RadiusMeasurement = Me.RadiusMeasurements.Where(Function(r) r.BladeId = y And Math.Round(r.Radius.Value) = Math.Round(Radius.Value)).FirstOrDefault()
#Enable Warning BC42324 ' Using the iteration variable in a lambda expression may have unexpected results
                        Dim localpitch As Double = GetLocalPitch(bladeData.CellMeasurements, Me.LocalPitchSectors, x, Me.PropellerDiameter, Me.Radius, Me.TEExclusion, Me.LEExclusion)
                        Dim poin = seriesPitch.Points.AddXY(y, Math.Round(localpitch, If(Precision, 2)))
                        seriesPitch.Points(poin).AxisLabel = kPointName + y.ToString()
                        seriesPitch.Points(poin).Color = GraphColorArray(y - 1)
                    Next
                Next
            End If
            MyBase.DataShow()
